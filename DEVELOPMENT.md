...
원본 좌표가 있어도 표 전체가 해당 줄 상자 높이에 포함되는 것은 아니다. `alignSourcePages`는 실제 표 높이가 다음 단위/본문 하단을 넘지 않는 경우에만 고정 배치를 사용한다. 넘으면 일반 흐름에서 간격을 계산하고 행 경계로 쪽을 나눈다. `renderPara`의 표 전용 문단은 글자용 내어쓰기 padding을 적용하지 않는다. 셀의 원본 줄이 셀 폭을 넘으면 `reflowOverflowingText`가 줄바꿈을 허용해 행 높이에 반영한다. 글자를 줄이거나 자르지 않는다.
...
저장된 줄 시작 위치는 표시 문자열만의 인덱스가 아니다. HWPX 분할 문단에서 fieldBegin/fieldEnd는 각각 8개의 UTF-16 위치를 차지하고 화면 문자를 만들지 않는다. tab도 8개 위치로 계산하며 보이는 문자와 원본 위치를 별도로 보관한다. 이 값은 문서별 보정량이 아니라 제어문자의 저장 단위다. 필드 parameters는 본문에 포함하지 않는다.

공통 Layout은 본문과 셀 모두에서 실제 글자 영역이 줄 경계를 넘을 때만 줄바꿈을 허용한다. 늘어난 줄 높이를 고정 좌표 충돌 판정에 반영하여 후속 문단과 겹치지 않게 일반 흐름/쪽 분할로 처리한다. 글자 배율이나 클리핑으로 넘침을 숨기지 않는다.
